...
void loop()
{
    ...
    if (runCount == 0)
    {
        ...
        if (MQTTconnect(mqttClient) == true) // Wysłanie danych przez protokół MQTT
        {
            ...
            MQTTPublish("IP", env.ip.c_str(), false, mqttClient);
            ...
            MQTTPublish("Maint", env.maint_mode, false, mqttClient);
